...
FTYPE eax_, eay_, eaz_, emx_, emy_, emz_;
...
void set_J(void) {
  ...
  MAT(J_, 0, 0) = -2 * (qw_ * bmx_ - qz_ * bmy_ + qy_ * bmz_);
  MAT(J_, 1, 0) = -2 * (qz_ * bmx_ + qw_ * bmy_ - qx_ * bmz_);
  MAT(J_, 2, 0) = -2 * (-qy_ * bmx_ + qx_ * bmy_ + qw_ * bmz_);
  MAT(J_, 3, 0) = -2 * (qw_ * bax_ - qz_ * bay_ + qy_ * baz_);
  MAT(J_, 4, 0) = -2 * (qz_ * bax_ + qw_ * bay_ - qx_ * baz_);
  MAT(J_, 5, 0) = -2 * (-qy_ * bax_ + qx_ * bay_ + qw_ * baz_);

  MAT(J_, 0, 1) = -2 * (qx_ * bmx_ + qy_ * bmy_ + qz_ * bmz_);
  MAT(J_, 1, 1) = -2 * (qy_ * bmx_ - qx_ * bmy_ - qw_ * bmz_);
  MAT(J_, 2, 1) = -2 * (qz_ * bmx_ + qw_ * bmy_ - qx_ * bmz_);
  MAT(J_, 3, 1) = -2 * (qx_ * bax_ + qy_ * bay_ + qz_ * baz_);
  MAT(J_, 4, 1) = -2 * (qy_ * bax_ - qx_ * bay_ - qw_ * baz_);
  MAT(J_, 5, 1) = -2 * (qz_ * bax_ + qw_ * bay_ - qx_ * baz_);

  MAT(J_, 0, 2) = -2 * (-qy_ * bmx_ + qx_ * bmy_ + qw_ * bmz_);
  MAT(J_, 1, 2) = -2 * (qx_ * bmx_ + qy_ * bmy_ + qz_ * bmz_);
  MAT(J_, 2, 2) = -2 * (-qw_ * bmx_ + qz_ * bmy_ - qy_ * bmz_);
  MAT(J_, 3, 2) = -2 * (-qy_ * bax_ + qx_ * bay_ + qw_ * baz_);
  MAT(J_, 4, 2) = -2 * (qx_ * bax_ + qy_ * bay_ + qz_ * baz_);
  MAT(J_, 5, 2) = -2 * (-qw_ * bax_ + qz_ * bay_ - qy_ * baz_);

  MAT(J_, 0, 3) = -2 * (-qz_ * bmx_ - qw_ * bmy_ + qx_ * bmz_);
  MAT(J_, 1, 3) = -2 * (qw_ * bmx_ - qz_ * bmy_ + qy_ * bmz_);
  MAT(J_, 2, 3) = -2 * (qx_ * bmx_ + qy_ * bmy_ + qz_ * bmz_);
  MAT(J_, 3, 3) = -2 * (-qz_ * bax_ - qw_ * bay_ + qx_ * baz_);
  MAT(J_, 4, 3) = -2 * (qw_ * bax_ - qz_ * bay_ + qy_ * baz_);
  MAT(J_, 5, 3) = -2 * (qx_ * bax_ + qy_ * bay_ + qz_ * baz_);
  // PD = J^T*J
...
void get_error(void) {
  ...
  MAT(e_, 0, 0) = emx_ - R11 * bmx_ - R12 * bmy_ - R13 * bmz_;
  MAT(e_, 1, 0) = emy_ - R21 * bmx_ - R22 * bmy_ - R23 * bmz_;
  MAT(e_, 2, 0) = emz_ - R31 * bmx_ - R32 * bmy_ - R33 * bmz_;

  MAT(e_, 3, 0) = eax_ - R11 * bax_ - R12 * bay_ - R13 * baz_;
  MAT(e_, 4, 0) = eay_ - R21 * bax_ - R22 * bay_ - R23 * baz_;
  MAT(e_, 5, 0) = eaz_ - R31 * bax_ - R32 * bay_ - R33 * baz_;
}
